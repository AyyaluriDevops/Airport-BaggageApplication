...
public class DiGraph<T extends Comparable<T>> {
	...
	public void add(T from, T to, int cost) {
		...
		if (temp != null) {
			...
			Edge oneDirection = new Edge(from, to, cost);
			edges.add(oneDirection);
			Edge biDirection = new Edge(to, from, cost);
			edges.add(biDirection);
		}
	}
	...
	/**
	 * Performs a recursive Depth First Search on the 'root' node (the first
	 * vertex created)
	 * 
	...
	/**
	 * Creates path information on the graph using the Dijkstra Algorithm, Puts
	 * the information into the Vertices, based on given starting vertex.
	 * 
	 * @param v1
	 *            Value of type T for starting vertex
	...
	private List<String> getShortestPath(Vertex target) {
		...
		for (Vertex v = target; v != null; v = v.previous) {
			path.add(v.value + " : " + v.minDistance);
		}
	...
	class Edge {
		...
		@Override
		public String toString() {
			return "Edge From: " + from.value + " to: " + to.value + " travelTime: " + cost;
		}
	}
}